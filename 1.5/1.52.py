...
def load_shortcuts(filename="abv.txt"):
    if not os.path.exists(filename):
        raise FileNotFoundError("ERROR: abv.txt not found in program directory.")
    ...
    with open(filename, "r", encoding="ascii", errors="strict") as f:
        for line_number, line in enumerate(f, 1):
            ...
            if "=" not in line:
                raise ValueError(f"Invalid format in abv.txt line {line_number}: '{line}'")
            ...
            if not full.islower() or not short.islower():
                raise ValueError(f"Line {line_number}: Only lowercase words allowed.")

            if not re.fullmatch(r"[a-z0-9]+", full):
                raise ValueError(f"Line {line_number}: Invalid characters in full word.")

            if not re.fullmatch(r"[a-z0-9]+", short):
                raise ValueError(f"Line {line_number}: Invalid characters in shortcut.")

            if full in shortcuts:
                raise ValueError(f"Duplicate full word on line {line_number}.")

            if short in shortcuts.values():
                raise ValueError(f"Duplicate shortcut on line {line_number}.")

            shortcuts[full] = short

    if not shortcuts:
        raise ValueError("abv.txt is empty. At least one shortcut required.")
...
def handle_compression(byte_data, final_filename):
    ...
    if compare == "y":
        if len(compressed_data) > len(byte_data):
            ...
            with open(final_filename, "wb") as f:
                f.write(compressed_data)
            print("Compressed file is smaller. Saved compressed.")
    else:
        with open(final_filename, "wb") as f:
            f.write(compressed_data)
        print("Saved compressed.")
...
def encode_to_file_from_text(txt_filename, output_filename):
    ...
    if errors:
        print(f"Found {len(errors)} errors. Fix input file.")
        return
